...
const { getAuthToken, searchEvents, getEventCategories } = require("./gigsbergApi");
...
async function createGigsbergListing(ticketId) {
  ...
  const price = Number(ticket.final_price || ticket.price || 0);
  ...
  /*
    ATTENZIONE:
    ticket_type_id va confermato con Gigsberg.
    Per ora usiamo env configurabile.
  */
  form.append("ticket_type_id", process.env.GIGSBERG_DEFAULT_TICKET_TYPE_ID || 3);
  ...
  /*
    ATTENZIONE:
    address_id e present_address_id sono richiesti per Paper Ticket.
    Se usi E-Ticket o Mobile Link potrebbero non servire.
    Li lasciamo configurabili.
  */
